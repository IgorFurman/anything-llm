...
const TemporaryAuthToken = {
  ...
  issue: async function (userId = null) {
    if (!userId) throw new Error("User ID is required to issue a temporary auth token.");
  ...
  invalidateUserTokens: async (userId) => {
    if (!userId) throw new Error("User ID is required to invalidate temporary auth tokens.");
   ...
   */
  validate: async (publicToken = "") => {
    /** @type {import("@prisma/client").temporary_auth_tokens & {user: import("@prisma/client").users} | undefined | null} **/
    let token;

    try {
      if (!publicToken)
        throw new Error("Public token is required to validate a temporary auth token.");
      ...
      if (token) await prisma.temporary_auth_tokens.delete({ where: { id: token.id } });
    }
  },
};
