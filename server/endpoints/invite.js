...
const { simpleSSOLoginDisabledMiddleware } = require("../utils/middleware/simpleSSOEnabled");
...
function inviteEndpoints(app) {
  ...
  app.get("/invite/:code", async (request, response) => {
    try {
      ...
      if (invite.status !== "pending") {
        response.status(200).json({ invite: null, error: "Invite is no longer valid." });
        return;
      }

      response.status(200).json({ invite: { code, status: invite.status }, error: null });
    } catch (e) {
      console.error(e);
  ...
  app.post("/invite/:code", [simpleSSOLoginDisabledMiddleware], async (request, response) => {
    try {
      const { code } = request.params;
