...
function apiDocumentEndpoints(app) {
  ...
  app.post("/v1/document/upload", [validApiKey, handleAPIFileUpload], async (request, response) => {
    /*
    ...
    */
    try {
      ...
      if (!success) {
        response.status(500).json({ success: false, error: reason, documents }).end();
        return;
      }
      ...
      if (!!addToWorkspaces)
        await Document.api.uploadToWorkspace(addToWorkspaces, documents?.[0].location);
      ...
      try {
        ...
        for (const doc of documents) {
          ...
          if (currentFolder !== folder) {
            ...
            const destinationPath = path.join(targetFolderPath, path.basename(doc.location));
  ...
  app.post("/v1/document/upload-link", [validApiKey], async (request, response) => {
    /*
    #swagger.tags = ['Documents']
    ...
    */
    try {
      const Collector = new CollectorApi();
      const { link, addToWorkspaces = "", scraperHeaders = {} } = reqBody(request);
      const processingOnline = await Collector.online();
      ...
      const { success, reason, documents } = await Collector.processLink(link, scraperHeaders);
      if (!success) {
        response.status(500).json({ success: false, error: reason, documents }).end();
        return;
      }
  ...
  app.post("/v1/document/raw-text", [validApiKey], async (request, response) => {
    /*
     #swagger.tags = ['Documents']
    ...
    try {
      ...
      const { textContent, metadata = {}, addToWorkspaces = "" } = reqBody(request);
      const processingOnline = await Collector.online();
      ...
      const { success, reason, documents } = await Collector.processRawText(textContent, metadata);
      if (!success) {
        response.status(500).json({ success: false, error: reason, documents }).end();
        return;
      }

      Collector.log(`Document created successfully. It is now available in documents.`);
  ...
  app.get("/v1/documents/folder/:folderName", [validApiKey], async (request, response) => {
    /*
    #swagger.tags = ['Documents']
  ...
  app.get("/v1/document/accepted-file-types", [validApiKey], async (_, response) => {
    /*
    #swagger.tags = ['Documents']
  ...
  app.get("/v1/document/metadata-schema", [validApiKey], async (_, response) => {
    /*
    #swagger.tags = ['Documents']
  ...
  app.post("/v1/document/create-folder", [validApiKey], async (request, response) => {
    /*
      #swagger.tags = ['Documents']
  ...
  app.delete("/v1/document/remove-folder", [validApiKey], async (request, response) => {
    /*
      #swagger.tags = ['Documents']
    ...
    try {
      ...
      response.status(200).json({ success: true, message: "Folder removed successfully" });
    } catch (e) {
      console.error(e);
      response.status(500).json({
        success: false,
  ...
  app.post("/v1/document/move-files", [validApiKey], async (request, response) => {
    /*
      #swagger.tags = ['Documents']
