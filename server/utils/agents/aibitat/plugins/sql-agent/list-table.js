    ...
    return {
      ...
      setup(aibitat) {
        aibitat.function({
          super: aibitat,
          name: this.name,
          description: "List all available tables in a database via its `database_id`.",
          examples: [
            {
            ...
            {
              prompt: "What information can you access in the customer_accts postgres db?",
          ...
          handler: async function ({ database_id = "" }) {
            try {
              ...
              if (!databaseConfig) {
                this.super.handlerProps.log(`sql-list-tables failed to find config!`, database_id);
              ...
              if (result.error) {
                this.super.handlerProps.log(`sql-list-tables tool reported error`, result.error);
