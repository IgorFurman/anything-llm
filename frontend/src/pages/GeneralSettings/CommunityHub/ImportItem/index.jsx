...
import CommunityHubImportItemSteps, { CommunityHubImportItemLayout } from "./Steps";

function SideBarSelection({ setStep, currentStep }) {
  const currentIndex = Object.keys(CommunityHubImportItemSteps).indexOf(currentStep);
  return (
    <div
    ...
    >
      {Object.entries(CommunityHubImportItemSteps).map(([stepKey, props], index) => {
        const isSelected = currentStep === stepKey;
        const isLast = index === Object.keys(CommunityHubImportItemSteps).length - 1;
        const isDone =
          currentIndex === Object.keys(CommunityHubImportItemSteps).length - 1 ||
          index < currentIndex;
        return (
          <div
            key={stepKey}
          ...
          >
            {isDone || isSelected ? (
              ...
              <div className="text-sm text-theme-text-secondary font-medium">{props.name}</div>
            )}
        ...
        <div className="flex flex-col w-full px-1 md:pl-6 md:pr-[86px] md:py-6 py-16">
          <div className="w-full flex flex-col gap-y-1 pb-6 border-white light:border-theme-sidebar-border border-b-2 border-opacity-10">
            ...
            <p className="text-xs leading-[18px] font-base text-theme-text-secondary">
              Import items from the AnythingLLM Community Hub to enhance your instance with
          ...
          <div className="flex-1 flex h-full">
            ...
            <div className="overflow-y-auto pb-[200px] h-screen no-scroll">
              <div className="ml-8">{StepPage.component({ settings, setSettings, setStep })}</div>
            </div>
          </div>
        </div>
      )}
